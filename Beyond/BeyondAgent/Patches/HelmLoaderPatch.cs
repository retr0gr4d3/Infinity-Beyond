...
namespace BeyondAgent.Patches
{
    ...
    public static class HelmSpoofPatch
    {
        ...
        {
            if (!BeyondAgentClass.helmSpoofActive || string.IsNullOrWhiteSpace(BeyondAgentClass.helmSpoofBundle))
            {
                return;
            }
            ...
            try
            {
                ...
                __result = BundleBuilder.Build(BeyondAgentClass.helmSpoofBundle, ItemCatalog.Helms, avt.character.Helm?.Bundle, __result);
            }
            catch (System.Exception ex)
            {
                BeyondLog.Error($"[HelmSpoofPatch] {ex.Message}");
            }
        }
    }
}
